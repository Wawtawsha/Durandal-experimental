...
class DurandalMCPServer extends EventEmitter {
    constructor(options = {}) {
        ...
        this.config = {
            ...
            transport: {
                type: options.transport || process.env.MCP_TRANSPORT || 'stdio',
                port: options.port || process.env.MCP_PORT || 8080,
        ...
        // Detect client type
        this.clientType = options.client || process.env.MCP_CLIENT || TransportFactory.detectClient();

        // Initialize protocol adapter if needed
        if (this.clientType === 'gemini') {
            this.protocolAdapter = new ProtocolAdapter('gemini', 'mcp', this.logger);
        } else if (this.clientType === 'openai') {
            this.protocolAdapter = new ProtocolAdapter('openai', 'mcp', this.logger);
        } else {
            this.protocolAdapter = null; // No adaptation needed for Claude
        }
    ...
    setupHandlers() {
        ...
        this.server.setRequestHandler(CallToolRequestSchema, async (request) => {
            // Apply protocol adaptation if needed (for Gemini/OpenAI clients)
            let adaptedRequest = request;
            if (this.protocolAdapter) {
                adaptedRequest = this.protocolAdapter.adaptRequest(request);
            }

            const toolName = adaptedRequest.params.name;
            const args = adaptedRequest.params.arguments || {};

            // Start MCP tool logging
            const requestId = this.logger.startMCPTool(toolName, args);

            try {
                this.logger.debug(`Processing tool: ${toolName}`, {
                    requestId,
                    args,
                    client: this.clientType
                });
                ...
                // Apply protocol adaptation to response if needed
                if (this.protocolAdapter) {
                    result = this.protocolAdapter.adaptResponse(result);
                }
    ...
    async start() {
        ...
        const transport = TransportFactory.create({
            ...
            apiKey: this.config.transport.apiKey,
            logger: this.logger
        });

        // For HTTP transport, we need to start it first
        if (transport.start) {
        ...
        this.logger.info('[START] Durandal MCP Server running', {
            transport: this.config.transport.type,
            client: this.clientType,
            port: this.config.transport.type === 'http' ? this.config.transport.port : undefined,
            features: ['logging', 'testing', 'debug', 'error-handling', 'multi-client']
        });
